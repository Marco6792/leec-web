...
const typeConfig: Record<string, { icon: typeof BookOpen; color: string; label: string }> = {
  journal: { icon: BookOpen, color: "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300", label: "Journal Article" },
  conference: { icon: Mic, color: "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300", label: "Conference Paper" },
  preprint: { icon: FileText, color: "bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300", label: "Preprint" },
  thesis: { icon: GraduationCap, color: "bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300", label: "Thesis" },
  report: { icon: Newspaper, color: "bg-slate-100 text-slate-700 dark:bg-slate-900/30 dark:text-slate-300", label: "Report" },
  book: { icon: BookOpen, color: "bg-rose-100 text-rose-700 dark:bg-rose-900/30 dark:text-rose-300", label: "Book" },
  chapter: { icon: FileText, color: "bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-300", label: "Book Chapter" },
  patent: { icon: Rocket, color: "bg-cyan-100 text-cyan-700 dark:bg-cyan-900/30 dark:text-cyan-300", label: "Patent" },
  software: { icon: Rocket, color: "bg-violet-100 text-violet-700 dark:bg-violet-900/30 dark:text-violet-300", label: "Software" },
  dataset: { icon: FileText, color: "bg-teal-100 text-teal-700 dark:bg-teal-900/30 dark:text-teal-300", label: "Dataset" },
};
...
export function PublicationView({
  ...
  const [newReview, setNewReview] = useState({ title: "", content: "", rating: 3, pros: "", cons: "" });
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 min-w-0 space-y-6">
          ...
          {publication.authors.length > 0 && (
            <div>
              <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-4">Authors</h3>
              <div className="flex flex-wrap gap-4">
                {publication.authors
                  .sort((a, b) => a.authorOrder - b.authorOrder)
                  .map((author) => {
                    const initials = author.fullName.split(" ").map((n) => n[0]).join("").slice(0, 2).toUpperCase();
                    return (
                      <Link
                        key={author.id}
                      ...
                      >
                        <div className="relative">
                          <div className="size-12 rounded-full overflow-hidden border-2 border-background bg-muted">
                            {author.avatarUrl ? (
                              <img src={author.avatarUrl} alt={author.fullName} className="size-full object-cover" />
                            ) : (
                              <div className="size-full flex items-center justify-center text-sm font-bold text-muted-foreground">{initials}</div>
                            )}
                          </div>
                        ...
                        <div>
                          <p className="text-sm font-semibold group-hover/av:text-primary transition-colors">{author.fullName}</p>
                          {author.title && <p className="text-xs text-muted-foreground">{author.title}</p>}
                          {author.affiliation && <p className="text-xs text-muted-foreground italic">{author.affiliation}</p>}
                        </div>
                      </Link>
                    );
                  })}
          ...
          {publication.abstract && (
            <div>
              <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-3">Abstract</h3>
              <p className="text-sm leading-relaxed text-muted-foreground whitespace-pre-line">{publication.abstract}</p>
            </div>
          )}
          ...
          {publication.pdfUrl && (
            ...
            >
              <div className="size-14 rounded-xl bg-primary/10 flex items-center justify-center shrink-0">
                <svg viewBox="0 0 24 24" className="size-7 text-primary" fill="none" stroke="currentColor" strokeWidth="1.5">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  {publication.openAccess ? "Open Access" : "View publication PDF"}
                </p>
              </div>
          ...
          {/* Other Links */}
          {(publication.doi || publication.codeUrl || publication.sourceDataUrl) && (
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {publication.sourceDataUrl && (
                  <a href={publication.sourceDataUrl} target="_blank" rel="noopener noreferrer"
                    className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg border text-sm hover:bg-muted transition-colors">
          ...
          <div>
            ...
            >
              ...
              {showComments ? <ChevronUp className="h-4 w-4 ml-auto" /> : <ChevronDown className="h-4 w-4 ml-auto" />}
            </button>
            {showComments && (
              <div className="mt-4 space-y-4">
                {currentUserId && (
                  <div className="flex gap-2">
                    ...
                    />
                    <Button size="sm" onClick={handleComment} disabled={submitting || !newComment.trim()} className="gap-1">
                      <Send className="h-3.5 w-3.5" />
                    </Button>
                  </div>
                )}
                {comments.length === 0 ? (
                  <p className="text-sm text-muted-foreground italic py-4 text-center">No comments yet. Be the first to comment.</p>
                ) : (
                  comments.map((comment) => {
                    const user = comment.user;
                    const ci = user?.fullName?.split(" ").map((n: string) => n[0]).join("").slice(0, 2).toUpperCase() ?? "??";
                    return (
                      <div key={comment.id} className="flex gap-3 p-4 rounded-xl border">
                        <div className="size-9 rounded-full overflow-hidden bg-muted shrink-0">
                          {user?.avatarUrl ? (
                            ...
                            <div className="size-full flex items-center justify-center text-xs font-bold text-muted-foreground">{ci}</div>
                          )}
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            {user?.id ? (
                              <Link href={`/profile/${user.id}`} className="text-sm font-semibold hover:text-primary transition-colors">
                                {user.fullName}
                              </Link>
                            ) : (
          ...
          <div>
            ...
            >
              ...
              {showReviews ? <ChevronUp className="h-4 w-4 ml-auto" /> : <ChevronDown className="h-4 w-4 ml-auto" />}
            </button>
            {showReviews && (
              <div className="mt-4 space-y-4">
                {currentUserId && (
                  <Card className="bg-muted/30">
                    <CardContent className="p-4 space-y-3">
                      ...
                      <Button size="sm" onClick={handleReview} disabled={submitting || !newReview.content.trim()} className="gap-1">
                        Submit Review
                      </Button>
                    </CardContent>
                  </Card>
                ...
                {reviews.length === 0 ? (
                  <p className="text-sm text-muted-foreground italic py-4 text-center">No reviews yet. Be the first to review.</p>
                ) : (
                  reviews.map((review) => {
                    const user = review.user;
                    const ri = user?.fullName?.split(" ").map((n: string) => n[0]).join("").slice(0, 2).toUpperCase() ?? "??";
                    return (
                      <Card key={review.id}>
                          ...
                          <div className="flex items-start gap-3 mb-3">
                            <div className="size-9 rounded-full overflow-hidden bg-muted shrink-0">
                              {user?.avatarUrl ? (
                                ...
                                <div className="size-full flex items-center justify-center text-xs font-bold text-muted-foreground">{ri}</div>
                              )}
                            </div>
                            <div className="flex-1 min-w-0">
                            ...
                            <div className="grid grid-cols-2 gap-3 text-xs">
                              {review.pros && (
                                <div className="p-2 rounded-lg bg-emerald-50 dark:bg-emerald-900/10">
                                  <span className="font-medium text-emerald-700 dark:text-emerald-300">Pros:</span>{" "}
                                  <span className="text-muted-foreground">{review.pros}</span>
                                </div>
                              )}
                              {review.cons && (
                                <div className="p-2 rounded-lg bg-rose-50 dark:bg-rose-900/10">
                                  <span className="font-medium text-rose-700 dark:text-rose-300">Cons:</span>{" "}
        ...
        <div className="lg:w-80 shrink-0 space-y-6">
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="text-center p-3 rounded-xl bg-muted/50">
                  <div className="text-2xl font-bold">{publication.citationCount ?? 0}</div>
                ...
                <div className="text-center p-3 rounded-xl bg-muted/50">
                  ...
                  <div className="text-[10px] text-muted-foreground uppercase tracking-wider">Ratings</div>
                </div>
                <div className="text-center p-3 rounded-xl bg-muted/50">
                  <div className="text-2xl font-bold">{reviews.length}</div>
                  <div className="text-[10px] text-muted-foreground uppercase tracking-wider">Reviews</div>
                </div>
              </div>
              {totalRatings > 0 && (
          ...
          <Card>
            <CardContent className="p-4 space-y-3">
              ...
              <Button variant="outline" className="w-full gap-2" disabled={!currentUserId}>
